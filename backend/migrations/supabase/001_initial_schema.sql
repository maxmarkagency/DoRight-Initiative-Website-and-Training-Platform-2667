...
This migration establishes the foundational database schema for the DoRight Learning Management System (LMS). It includes tables for managing users, courses, content, enrollments, progress, assessments, payments, and community features.

## 1. Extensions and Custom Types
- Enables the `uuid-ossp` extension for UUID generation.
- Defines custom ENUM types for various statuses and roles to ensure data consistency:
  - `user_role`: student, instructor, admin, staff
  - `course_status`: draft, published, archived
  - `enrollment_status`: active, completed, cancelled, expired
  - `content_type`: video, text, pdf, quiz, assignment, external_link
  - `question_type`: mcq, multiple_choice, true_false, short_answer, long_answer, fill_blank
  - `payment_status`: pending, completed, failed, cancelled, refunded
  - `submission_status`: in_progress, submitted, graded, returned

## 2. New Tables
- **users**: Extends Supabase `auth.users` with profile information like name, role, and bio.
- **categories**: For organizing courses.
- **courses**: Core table for course information, including title, description, instructor, and pricing.
...
- **progress**: Monitors user progress through lessons.
- **quizzes**: Defines quizzes for courses/lessons.
- **questions**: Stores questions for quizzes.
- **quiz_submissions**: Records user attempts on quizzes.
- **assignments**: Defines assignments for courses/lessons.
- **assignment_submissions**: Records user submissions for assignments.
- **certificates**: Stores certificates issued to users upon course completion.
- **media**: Manages uploaded media files.
- **payments**: Tracks payment transactions for course enrollments.
- **announcements**: For course-specific announcements.
- **discussions**: A forum system for courses.
- **discussion_replies**: Replies within discussion threads.
- **reviews**: User ratings and reviews for courses.
- **settings**: Key-value store for site-wide settings.

## 3. Indexes and Triggers
- Creates numerous indexes on foreign keys and frequently queried columns to optimize database performance.
- Implements a trigger function (`update_updated_at_column`) to automatically update the `updated_at` timestamp on relevant tables when a record is modified.
*/
