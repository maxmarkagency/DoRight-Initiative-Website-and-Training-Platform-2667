...
-- Modules policies
CREATE POLICY "Modules viewable for published courses" ON modules
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Course instructors can manage modules" ON modules
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM courses 
...
-- Lessons policies
CREATE POLICY "Lessons viewable for enrolled users or preview lessons" ON lessons
  FOR SELECT USING (
    is_preview = true OR
    EXISTS (
...
CREATE POLICY "Instructors can view quiz submissions for their courses" ON quiz_submissions
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Instructors can grade quiz submissions for their courses" ON quiz_submissions
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM quizzes q
      JOIN courses c ON c.id = q.course_id
...
CREATE POLICY "Instructors can view assignment submissions for their courses" ON assignment_submissions
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Instructors can grade assignment submissions for their courses" ON assignment_submissions
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM assignments a
      JOIN courses c ON c.id = a.course_id
...
CREATE POLICY "Instructors can view reviews for their courses" ON reviews
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM courses 
      WHERE id = course_id AND instructor_id = auth.uid()
...
-- Discussion policies
CREATE POLICY "Enrolled users can view course discussions" ON discussions
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM enrollments 
...
CREATE POLICY "Users can view replies for viewable discussions" ON discussion_replies
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE d.id = discussion_id AND (
        ...
        EXISTS (
          SELECT 1 FROM courses c
          WHERE c.id = d.course_id AND c.instructor_id = auth.uid()
        ) OR
        EXISTS (
          SELECT 1 FROM users 
          WHERE id = auth.uid() AND role IN ('admin', 'staff')
...
CREATE POLICY "Users can create replies to viewable discussions" ON discussion_replies
  FOR INSERT WITH CHECK (
    auth.uid() = user_id AND
    EXISTS (
      SELECT 1 FROM discussions d
