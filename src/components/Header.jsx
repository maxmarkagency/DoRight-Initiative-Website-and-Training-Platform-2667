        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center">
            <div className="hidden md:flex items-center space-x-4">
              <Link to="/join" className="group flex items-center px-4 py-2 border border-yellow-400 text-sm font-medium rounded-md text-yellow-400 hover:bg-yellow-400 hover:text-black transition-colors">
                Join Us
                <SafeIcon icon={FiArrowRight} className="ml-2 h-4 w-4 transform transition-transform duration-300 group-hover:translate-x-1" />
              </Link>
              {loading ? (
                ...
                <Link to="/login" className="flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-black bg-yellow-400 hover:bg-yellow-500 transition-colors">
                  <SafeIcon icon={FiLogIn} className="mr-2 -ml-1 h-5 w-5" />
                  Login
                </Link>
              )}
            </div>
            ...
            <div className="md:hidden ml-4">
              <button onClick={toggleMenu} className="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
                ...
                <SafeIcon icon={isOpen ? FiX : FiMenu} className="block h-6 w-6" />
              </button>
            </div>
          </div>
        </div>
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navLinks.map((item) => (
                item.dropdown ? (
                  <div key={item.name}>
                    <h3 className="px-4 py-2 text-sm font-semibold text-gray-400">{item.name}</h3>
                    {item.dropdown.map(subLink => (
                  ...
                  <NavLink key={item.name} to={item.path} onClick={toggleMenu}>{item.name}</NavLink>
                )
              ))}
              <div className="border-t border-gray-700 pt-4 mt-4 space-y-2">
                <NavLink to="/join" onClick={toggleMenu}>Join Us</NavLink>
                {loading ? (
                  <div className="flex items-center px-4">
                    <div className="w-10 h-10 bg-gray-700 rounded-full animate-pulse"></div>
                    <div className="ml-3 h-4 bg-gray-700 rounded w-24 animate-pulse"></div>
                  </div>
                ) : user ? (
                  ...
                  <NavLink to="/login" onClick={toggleMenu}>
                    <div className="flex items-center">
                      <SafeIcon icon={FiLogIn} className="mr-2 h-5 w-5" />
                      Login
                    </div>
                  </NavLink>
                )}
