/*
# Add Blog and Gallery Management

This migration introduces new tables and policies to support managing blog posts and gallery items within the admin section.

## 1. New Custom Types
- `blog_status`: Defines the status of a blog post (`draft`, `published`, `archived`).
- `gallery_media_type`: Defines the type of media in the gallery (`image`, `video`).

## 2. New Tables
- **blog_posts**: Stores all blog post content, including title, content, author, and publication status.
- **gallery_items**: Manages media for the gallery, including images and videos, with details like title, description, and media URLs.

## 3. Security (RLS)
- Enables RLS on the new tables.
- **blog_posts Policies**:
  - Public users can only read `published` posts.
  - Admins and staff can manage all blog posts (create, read, update, delete).
- **gallery_items Policies**:
  - All gallery items are publicly viewable.
  - Admins and staff can manage all gallery items.

## 4. Triggers
- Adds triggers to automatically update the `updated_at` timestamp for the `blog_posts` table.
*/
